...
# 检查输入参数
if [ $# -ne 2 ]; then
    echo "Usage: $0 <kernel_file> <disk_image>"
    exit 1
fi
...
# 拷贝内核文件到分区根目录
echo "Copying the kernel file to the root directory of the partition..."
sudo cp "$KERNEL_FILE" "$MOUNT_DIR/"

if [ $? -ne 0 ]; then
    echo "Error: Failed to copy the kernel file!"
...
# 输出完成信息
echo "Kernel file installation completed!"
